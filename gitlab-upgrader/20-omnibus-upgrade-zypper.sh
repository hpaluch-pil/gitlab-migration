#!/bin/bash
# 20-omnibus-upgrade-zypper.sh - upgrade just GitLab package (Omnibus) - for Zypper distributions
# should be done only after ./10-os-updates-zypper.sh
# NOTE: If you are using proxy wrapper script you can use this command:
# $ PROXY_WRAPPER=proxy_command.sh ./20-omnibus-upgrade-zypper.sh
set -euo pipefail

# expected distribution class (ID_LIKE in /etc/os-release)
EXP_ID_LIKE="suse opensuse"
OS_STATE_DIR=$HOME/.config/gitlab-upgrader/10-os-updates-zypper
STATE_DIR=$HOME/.config/gitlab-upgrader/$(basename -s .sh "$0")
# Timestamp - must be unique per day
TS="$(date '+%F')"

# prints "debian" for Debian and Ubuntu, "suse opensuse" for openSUSE LEAP
print_id_like ()
{
	source /etc/os-release && echo "$ID_LIKE"
}

verbose_command ()
{
	echo "Running '$@'..."
	"$@"
}

wait_cpu_down ()
{
	local max_cpu=20
	# CPU usage must be below $max_cpu otherwise gitlab-ce is starting up (should not be interrupted):
	echo "Waiting for CPU user% < $max_cpu% (gitlab-ce must be down or NOT starting)..."
	while true
	do
		cpu_usage=$(vmstat 1 2 | tail -1 | awk '{ print $(NF-4) }')
		[[ $cpu_usage =~ ^[0-9]+$ ]] || {
			echo "ERROR: CPU usage ($cpu_usage) must be number" >&2
			exit 1
		}
		echo "$(date '+%T') CPU user: $cpu_usage% (must be < $max_cpu%)"
		[ $cpu_usage -ge $max_cpu ] || break
		verbose_command sleep 5
	done
}


# optional command to wrap proxy access, using ${var:-} to avoid undefined variable error (set -u)
PROXY_WRAPPER="${PROXY_WRAPPER:-}"

# verify that 10-os-updates-zypper.sh finished OS updates:
fos=$OS_STATE_DIR/$TS-20-finished
[ -f "$fos" ] || {
	echo "ERROR: You have to run ./10-os-updates-zypper.sh first" >&2
	exit 1
}

# ff=finished file
ff="$STATE_DIR/$TS-99-finished"
[ ! -f "$ff" ] || {
	echo "ERROR: Upgrade is already finished for today - state file '$ff' already exists." >&2
	exit 1
}

cd "$(dirname $0)"
echo "INFO: User=`id -un`, UID=`id -u`"

[ `id -u` -ne 0 ] || {
	echo "ERROR: This script may NOT be run as root (it uses sudo internally)" >&2
	exit 1
}

# NOTE: We do NOT check for "zypper" command because some distributions
# may install foreign package manager confusing this script.
os_class="$(print_id_like)"
echo "INFO: OS class is: '$os_class'"
[ "$os_class" == "$EXP_ID_LIKE" ] || {
	echo "ERROR: got OS class '$os_class' but expected: '$EXP_ID_LIKE'" >&2
	exit 1
}

[ -d "$STATE_DIR" ] || verbose_command mkdir -p "$STATE_DIR"

# example output of "rpm -q gitlab-ce": gitlab-ce-17.8.7-ce.0.sles15.x86_64
gl_ver="$(rpm -q gitlab-ce)"

echo "INFO: installed gitlab-ce version: '$gl_ver'"
# valid version string: 17.8.7-ce.0
[[ $gl_ver =~ ^gitlab-ce-([0-9]+)\.([0-9]+)\.([0-9]+)-ce\.0.sles ]] || {
	echo "ERROR: GitLab version '$gl_ver' has unexpected format" >&2
	exit 1
}
gl_major=${BASH_REMATCH[1]}
gl_minor=${BASH_REMATCH[2]}
gl_patch=${BASH_REMATCH[3]}
echo "INFO: GitLab version triplet: '$gl_major', '$gl_minor', '$gl_patch'"
gl_ver_normalized=$(( gl_major * 10000 + gl_minor * 100 + gl_patch ))
echo "INFO: GitLab normalized version: '$gl_ver_normalized'"

# n=next + 1 gitlab version in zypper form : 17.8
# NOTE: We add +2 because we will define exclusion version rule for zypper
gln=$gl_major.$(( gl_minor + 2 ))
echo "INFO: Next+1 GitLab version: '$gln'"
[[ $gln =~ ^([0-9]+)\.([0-9]+)$ ]] || {
	echo "GitLab next+1 ver; '$gln' has unexpected format (should be like 17.8.*" >&2
	exit 1
}

# remove all "gitlab-ce" locks including "gitlab-ce >= X.Y"
for i in $(zypper -t ll | awk '$3 == "gitlab-ce" { print $1 }' | sort -unr)
do
	echo "INFO: Removing gitlab-ce lock No. '$i'"
	[[ $i =~ ^[0-9]+$ ]] || {
		echo "ERROR: Lock ID '$i' is not number" >&2
		exit 1
	}
	verbose_command sudo zypper rl "$i"
done
# add lock to target GitLab version+1 (+1, because >= operator for exclusion)
verbose_command sudo zypper al "gitlab-ce >= $gln"
verbose_command zypper ll

svc=gitlab-runsvdir.service
# GitLab must be in running state
gl_state="$(systemctl show -p SubState --value $svc)"
echo "Checking if $svc is running (state: $gl_state)"
[ "$gl_state" = running ] || {
	echo "ERROR: $svc is in state '$gl_state', expected 'running'" >&2
	exit 1
}

verbose_command	../is_gitlab_ready.sh

verbose_command sudo $PROXY_WRAPPER zypper --verbose in --dry-run gitlab-ce
echo -n "Are you sure to upgrade gitlab-ce package [y/N]? "
read ans
case "$ans" in 
	[yY]|[yY][eE][sS])
		verbose_command sudo $PROXY_WRAPPER zypper --verbose in gitlab-ce
		;;
	*)
		echo "ABORTED on user request"
		exit 1
		;;
esac
# immediately lock gitlab-ce package again to prevent upgrades by mistake...
verbose_command sudo zypper al gitlab-ce
verbose_command zypper ll

# CPU usage must be below 20% otherwise gitlab-ce is starting up (should not be interrupted):
wait_cpu_down

# mark that GitLab package upgrade is completed
verbose_command touch "$ff"

echo "OK: now poll ../is_gitlab_ready.sh script for finishing migrations"
exit 0
