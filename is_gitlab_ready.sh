...
gitlab_bg_migration_completed ()
{
	...
	set -x
	$sd gitlab-psql -c 'select job_class_name, table_name, column_name, job_arguments,now()-started_at as elapsed_time from batched_background_migrations where status not in (3,6)' | tee $log
	set +x
	fgrep -q '(0 rows)' $log || {
